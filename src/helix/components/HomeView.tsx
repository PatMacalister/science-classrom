...
import type { Unit } from "@/helix/lib/curriculum/types";
import { LESSONS, UNITS, getUnit, lessonNumber, lessonsOfUnit } from "@/helix/lib/curriculum/registry";
import { TIERS, tierIdOf } from "@/helix/lib/curriculum/tiers";
import { localizeLesson, localizeUnit } from "@/helix/lib/curriculum/localize";
  ...
  return (
    <>
      <section className="hero">
        ...
        <Link className="btn" href={`/helix/lesson/${(firstIncomplete ?? LESSONS[0]).slug}`}>
          {doneCount === 0 ? t("startLearning") : t("continueLearning")}
        </Link>{" "}
        <Link className="btn secondary" href="/helix/codons">
          {t("codonTable")}
        </Link>{" "}
        <Link className="btn secondary" href="/helix/review" suppressHydrationWarning>
          {t("reviewDeck")}{reviewDue > 0 ? ` ${t("reviewDue", { n: reviewDue })}` : ""}
        </Link>{" "}
        <Link className="btn secondary" href="/helix/exam">
          {t("examLink")}
        </Link>{" "}
        <Link className="btn secondary" href="/helix/stats">
          {t("statsLink")}
        </Link>{" "}
        <Link className="btn secondary" href="/helix/certificate">
          {t("certificate")}
        </Link>
      </section>
