        ...
        <div className="lesson-grid">
          {lessons.map((lessonRaw) => {
            ...
            return (
              <Link
                key={lesson.slug}
                href={`/catalyst/lesson/${lesson.slug}`}
    ...
    <>
      <section className="hero">
        ...
        <div className="hero-stats">
          ...
          <span className="stat-chip">
            <b>118</b> {t("statElements")}
          </span>
          <span className="stat-chip" suppressHydrationWarning>
            <b>{progress.ready ? doneCount : "…"}</b> {t("statCompleted")}
          </span>
        </div>
        <Link className="btn" href={`/catalyst/lesson/${(firstIncomplete ?? LESSONS[0]).slug}`}>
          {doneCount === 0 ? t("startLearning") : t("continueLearning")}
        </Link>{" "}
        <Link className="btn secondary" href="/catalyst/elements">
          {t("periodicTable")}
        </Link>{" "}
