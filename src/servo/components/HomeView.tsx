...
import type { Unit } from "@/servo/lib/curriculum/types";
import { LESSONS, UNITS, getUnit, lessonNumber, lessonsOfUnit } from "@/servo/lib/curriculum/registry";
import { TIERS, tierIdOf } from "@/servo/lib/curriculum/tiers";
import { localizeLesson, localizeUnit } from "@/servo/lib/curriculum/localize";
import { useLang, useT, type UIKey } from "@/servo/lib/i18n";
import { useProgress } from "@/shared/progress";
import { PART_ENTRIES } from "@/servo/lib/parts";
    ...
    <>
      <section className="hero">
        ...
        <div className="hero-stats">
          ...
          <span className="stat-chip">
            <b>{PART_ENTRIES.length}</b> {t("statSi")}
          </span>
          <span className="stat-chip" suppressHydrationWarning>
            <b>{progress.ready ? doneCount : "…"}</b> {t("statCompleted")}
          </span>
        </div>
        <Link className="btn" href={`/servo/lesson/${(firstIncomplete ?? LESSONS[0]).slug}`}>
          {doneCount === 0 ? t("startLearning") : t("continueLearning")}
        </Link>{" "}
        <Link className="btn secondary" href="/servo/parts">
          {t("siTable")}
        </Link>{" "}
        <Link className="btn secondary" href="/servo/review" suppressHydrationWarning>
          {t("reviewDeck")}{reviewDue > 0 ? ` ${t("reviewDue", { n: reviewDue })}` : ""}
        </Link>{" "}
        <Link className="btn secondary" href="/servo/exam">
          {t("examLink")}
        </Link>{" "}
        <Link className="btn secondary" href="/servo/stats">
          {t("statsLink")}
        </Link>{" "}
        <Link className="btn secondary" href="/servo/certificate">
          {t("certificate")}
        </Link>
      </section>
