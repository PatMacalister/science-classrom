...
- **Language** (EN/DE) is one shared preference (`science-lang`) across the
  landing page and every course. Spark and Catalyst are fully bilingual:
  interface, every lesson body (theory, quizzes, problems, checklists) and the
  canvas labels inside the simulations. Helix ships a German interface, German
  unit/lesson metadata and German canvas labels; its lesson *bodies* are still
  English and show the built-in fallback notice — that is the remaining work on
  the newest course, and `curriculum/de/unitN.tsx` is where it goes.
...
src/
  ...
  helix/                   The complete Helix course (components + lib):
                           ...
                           DE Eselsbrücke), curriculum/de/meta.ts (German
                           titles; bodies still English), labStrings.ts
